...
!(function($) {
	$.base("base.app", {
		init: function(arg) {	
			this.moduleData = {};
			this.sandbox = sandbox.newInstance();
			this.lazymodule = lazymodule.newInstance(this.sandbox);
			return this;
		},
		...
		*/
		lazyRegister: function(moduleId, creator, els, event, opt){
			...
			try {
				...
				if(!DEBUG_MOD){
					perfmjs.logger.error("could not lazyregister " + moduleId + " because: " + ex.message);
				}else{
					throw ex;
				}
				return false;
		...
		*/
		start: function(moduleId){
			...
			try {
				...
				perfmjs.logger.info(moduleId + " init finished, cost:" + + (perfmjs.utils.now() - start) + " ms");
				return true;
			} catch (ex) {
				if(!DEBUG_MOD){
		...
		*/		
		startAll: function(){
			...
			this.sandbox.notify(this.options['allReadyEvent']);
			return _results;		
		},
		...
		*/		
		stop:function(moduleId) {
			...
			if (module.instance) {
				if(perfmjs.utils.isFunction(module.instance.destroy)){
...
//			var sb = sandbox.newInstance(this, moduleId, opt);
		...
		_getModuleConfig:function(moduleId, opt){
			var domId = "#" + moduleId;
			var domNode = jquip(domId);
			if(domNode.length>0){
				//从dom节点中获取埋入的后端数据 data-conf={"date":"1965-09-01","time":"19:30"}
				var data = domNode.attr(this.options['dataField']);
				if(data){
					opt.data = perfmjs.utils.parseJSON(data);
				}
				//从dom节点中找到对应的bigRender优化方式下用<script>标签隐藏的dom节点，插回到正常dom结构中。
				var config = domNode.attr(this.options['scriptField']);
				if(config){
					var object = perfmjs.utils.parseJSON(config);
					var script = jquip("#" +object.script);
					if(script.length>0){
						if(object.replace){
		...
		_getModuleCombine:function(moduleId, opt){
			if(typeof(opt.module) !== 'undefined') {
				//从dom节点中获取模块展示的组合号 combine-conf="combine1"
				//大多数应用中是不需要combine-conf节点的，但一些复杂应用，一个模块会有
				//不同的展示方式，对应的js、css文件也就不同，因此需要通过获取dom上的埋点来确定加载哪些文件。
				var domId = "#" + moduleId;
				var domNode = jquip(domId);
				if(domNode.length>0){
					var combine = domNode.attr(this.options['combineField']);
					if(combine){
		...
		_parseFunction:function(s){
			...
			l=a.length,
			o = window;
			for (var i=(perfmjs.utils.isWindow(a[0])?1:0);i<l;i++) {
				if(perfmjs.utils.isFunction(o[a[i]]) || perfmjs.utils.isPlainObject(o[a[i]])) {
					o = o[a[i]];
				} else {
					return null;
				}
			}
			if(perfmjs.utils.isFunction(o)){
				return o;
			}
			return null;
		},
